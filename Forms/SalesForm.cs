...
public sealed class SalesForm : Form
{
    ...
    private void BuildUi()
    {
        ...
        shell.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 112F));
        shell.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100F));

        shell.Controls.Add(BuildActionRail(), 0, 0);
    ...
    private Control BuildActionRail()
    {
        ...
        rail.RowStyles.Add(new RowStyle(SizeType.Absolute, 82F));
        for (var i = 1; i < 9; i++) rail.RowStyles.Add(new RowStyle(SizeType.Absolute, 58F));
        rail.RowStyles.Add(new RowStyle(SizeType.Percent, 100F));

        AddRailButton(rail, 0, "✚\r\nجديد", UiTheme.PrimaryAction, ClearCurrentInvoice);
        AddRailButton(rail, 1, "💾\r\nحفظ", UiTheme.SecondaryAction, SaveOrder);
        AddRailButton(rail, 2, "↻\r\nإلغاء", UiTheme.WarningAction, ClearCurrentInvoice);
        AddRailButton(rail, 3, "＋\r\nسطر جديد", UiTheme.Accent, AddNewInvoiceLine);
        AddRailButton(rail, 4, "−\r\nحذف سطر", UiTheme.DangerAction, DeleteSelectedLine);
        AddRailButton(rail, 5, "⌕\r\nبحث صنف", UiTheme.Accent, OpenIndependentProductSearch);
        AddRailButton(rail, 6, "▣\r\nبطاقة صنف", UiTheme.SecondaryAction, OpenProductCard);
        AddRailButton(rail, 7, "🖨\r\nطباعة", UiTheme.SecondaryAction, () => ShowInfo("الطباعة غير مرتبطة بعد بوظيفة في شاشة البيع."));
        AddRailButton(rail, 8, "✕\r\nإغلاق", UiTheme.DangerAction, Close);
        return rail;
    }

    private static void AddRailButton(TableLayoutPanel rail, int row, string text, Color color, Action action)
    {
    ...
    private Control BuildInvoiceHeader()
    {
        var header = new TableLayoutPanel
        {
            ...
            RightToLeft = RightToLeft.Yes, BackColor = Color.White
        };
        ...
        {
            if (e.KeyCode == Keys.Enter) { AddFirstSearchResult(); e.SuppressKeyPress = true; }
        };
        ...
        add.Dock = DockStyle.Fill; add.Margin = new Padding(2); add.Click += (_, _) => AddFirstSearchResult();
        entry.Controls.Add(add, 6, 0);
        var search = InvoiceUi.ActionButton("بحث أصناف", UiTheme.Accent);
        search.Dock = DockStyle.Fill; search.Margin = new Padding(2); search.Click += (_, _) => OpenIndependentProductSearch();
    ...
    private Control BuildInvoiceSummary()
    {
        ...
        {
            Dock = DockStyle.Fill, ColumnCount = 8, RowCount = 2,
            Padding = new Padding(0, 2, 0, 2), Margin = Padding.Empty,
            RightToLeft = RightToLeft.Yes, BackColor = Color.FromArgb(244, 247, 244)
        };
        ...
        for (var i = 0; i < titles.Length; i++)
        {
            ...
            if (i == 7) cell.Text = _invoiceCountLabel.Text = "0";
    ...
    private void ConfigureGrid()
    {
        ...
        _grid.KeyDown += (_, e) =>
        {
            if (e.KeyCode == Keys.Insert) { AddNewInvoiceLine(); e.SuppressKeyPress = true; }
            else if (e.KeyCode == Keys.Delete && _grid.CurrentRow is not null) { DeleteSelectedLine(); e.SuppressKeyPress = true; }
        };

        // Add the columns in reverse collection order so the RTL DataGridView
        // presents the visual order from RIGHT -> LEFT without touching the data model.
    ...
    private void AddGridColumn(string header, string property, float weight)
    {
        ...
        if (property is "Quantity" or "Balance" or "UnitPrice" or "DiscountValue" or "AfterDiscount") column.DefaultCellStyle.Format = "N2";
    ...
    private Control BuildBottom()
    {
        ...
        _totalLabel.Text = "الإجمالي: 0.00"; _totalLabel.Dock = DockStyle.Fill; _totalLabel.Font = new Font("Segoe UI", 16F, FontStyle.Bold); _totalLabel.ForeColor = UiTheme.InvoiceGreen; _totalLabel.TextAlign = ContentAlignment.MiddleRight; _totalLabel.Padding = new Padding(6); bottom.Controls.Add(_totalLabel, 0, 0);
        ...
        discountBox.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 52F)); discountBox.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100F));
        discountBox.Controls.Add(CreateFieldLabel("الخصم"), 0, 0);
        _discount.DecimalPlaces = 2; _discount.Maximum = 100000000M; _discount.Dock = DockStyle.Fill; _discount.Margin = new Padding(4); UiTheme.StyleInput(_discount); _discount.TextAlign = HorizontalAlignment.Right; _discount.ValueChanged += (_, _) => UpdateTotal();
        discountBox.Controls.Add(_discount, 1, 0); bottom.Controls.Add(discountBox, 1, 0);
        _costLabel.Text = "تكلفة الفاتورة: 0.00"; _costLabel.Dock = DockStyle.Fill; _costLabel.TextAlign = ContentAlignment.MiddleRight; _costLabel.Font = new Font("Segoe UI", 9.5F, FontStyle.Bold); bottom.Controls.Add(_costLabel, 2, 0);
        _profitLabel.Text = "قيمة ربح الفاتورة: 0.00"; _profitLabel.Dock = DockStyle.Fill; _profitLabel.TextAlign = ContentAlignment.MiddleRight; _profitLabel.Font = new Font("Segoe UI", 9.5F, FontStyle.Bold); _profitLabel.ForeColor = Color.FromArgb(0, 120, 0); bottom.Controls.Add(_profitLabel, 3, 0);
        _cashierLabel.Text = "المستخدم الحالي: Administrator"; _cashierLabel.Dock = DockStyle.Fill; _cashierLabel.TextAlign = ContentAlignment.MiddleRight; _cashierLabel.Font = new Font("Segoe UI", 8.5F); bottom.Controls.Add(_cashierLabel, 0, 1);
        _notes.Multiline = true; _notes.ScrollBars = ScrollBars.Vertical; _notes.Dock = DockStyle.Fill; _notes.Margin = new Padding(4); UiTheme.StyleInput(_notes); _notes.TextAlign = HorizontalAlignment.Right; _notes.RightToLeft = RightToLeft.Yes; _notes.PlaceholderText = "ملاحظات"; bottom.Controls.Add(_notes, 1, 1); bottom.SetColumnSpan(_notes, 2);
        var save = InvoiceUi.ActionButton("حفظ فاتورة البيع", UiTheme.PrimaryAction); save.Dock = DockStyle.Fill; save.Margin = new Padding(4); save.Click += (_, _) => SaveOrder(); bottom.Controls.Add(save, 3, 1);
        return bottom;
    }

    private static void AddFieldPair(TableLayoutPanel parent, int startColumn, string labelText, Control value)
    {
        parent.Controls.Add(CreateFieldLabel(labelText), startColumn, 0); parent.Controls.Add(value, startColumn + 1, 0); parent.SetColumnSpan(value, 1);
    }

    private static TextBox CreateReadonlyValue(string text)
    {
        var value = new TextBox { Text = text, ReadOnly = true, Dock = DockStyle.Fill, TextAlign = HorizontalAlignment.Right, Margin = new Padding(2), RightToLeft = RightToLeft.Yes, BackColor = Color.White };
        UiTheme.StyleInput(value); return value;
    }

    private static void PrepareCombo(ComboBox combo)
    {
        combo.Dock = DockStyle.Fill; combo.DropDownStyle = ComboBoxStyle.DropDownList; combo.RightToLeft = RightToLeft.Yes; combo.Margin = new Padding(2); UiTheme.StyleInput(combo);
    }

    private static Label CreateFieldLabel(string text) => InvoiceUi.FieldLabel(text);

    private void OpenIndependentProductSearch() { using var dialog = new ProductSearchDialog(); dialog.ShowDialog(this); }

    private void OpenProductCard()
    {
        long? productId = null;
        if (_grid.CurrentRow?.Index >= 0 && _grid.CurrentRow.Index < _items.Count)
        {
            var item = _items[_grid.CurrentRow.Index];
            if (item.ProductId > 0) productId = item.ProductId;
        }
        if (!productId.HasValue)
        {
            ...
            if (!string.IsNullOrWhiteSpace(query))
            {
                ...
                command.Parameters.AddWithValue("$exact", query); command.Parameters.AddWithValue("$like", $"%{query}%");
                var result = command.ExecuteScalar(); if (result is not null) productId = Convert.ToInt64(result);
            }
        }
        if (!productId.HasValue) { MessageBox.Show(this, "حدد صنفًا أو اكتب اسم الصنف أولًا.", "بيانات الصنف", MessageBoxButtons.OK, MessageBoxIcon.Information); return; }
        using var card = new ProductCardDialog(productId.Value); card.ShowDialog(this);
    }
    ...
    private void LoadCustomers()
    {
        try
        {
            ...
            _customer.DataSource = list; _customer.DisplayMember = nameof(CustomerChoice.DisplayName); _customer.ValueMember = nameof(CustomerChoice.Id);
            _paymentStatus.Items.Clear(); _paymentStatus.Items.AddRange(new object[] { "كاش", "فيزا", "آجل", "محفظة إلكترونية", "تحويل بنكي" }); _paymentStatus.SelectedIndex = 0;
        }
    ...
    {
        if (_items.Any(x => x.ProductId == 0))
        {
            _productSearch.Focus();
            return;
        }
        _items.Add(new SalesOrderItemDraft { ProductId = 0, ProductName = "", Quantity = _quantity.Value, UnitPrice = 0m, CostPrice = 0m });
        RefreshGrid();
        _grid.CurrentCell = _grid.Rows.Count > 0 ? _grid.Rows[^1].Cells[0] : null;
        if (_grid.CurrentRow is not null) _grid.CurrentRow.Selected = true;
        _productSearch.Focus();
    }
    ...
    private void AddFirstSearchResult()
    {
        ...
        try
        {
            ...
            command.Parameters.AddWithValue("$q", $"%{query}%"); command.Parameters.AddWithValue("$exact", query);
            using var reader = command.ExecuteReader();
            if (!reader.Read()) { MessageBox.Show(this, "الصنف غير موجود.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
            var id = reader.GetInt64(0); var name = reader.IsDBNull(1) ? "" : reader.GetString(1); var sell = reader.IsDBNull(2) ? 0m : Convert.ToDecimal(reader.GetValue(2)); var buy = reader.IsDBNull(3) ? 0m : Convert.ToDecimal(reader.GetValue(3));
            var blank = _items.FirstOrDefault(x => x.ProductId == 0);
            if (blank is not null) { blank.ProductId = id; blank.ProductName = name; blank.Quantity = _quantity.Value; blank.UnitPrice = sell; blank.CostPrice = buy; }
            else
            {
            ...
            _productSearch.Clear(); _quantity.Value = 1M; RefreshGrid(); _productSearch.Focus();
        }
        catch (Exception ex) { MessageBox.Show(this, $"تعذر إضافة الصنف:\n{ex.Message}", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error); }
    }
    ...
    private void RefreshGrid()
    {
        ...
        {
            x.ProductId, x.ProductName, Expiry = "", Unit = "علبة", x.Quantity, Balance = 0m, x.UnitPrice, Reorder = 0m,
            DiscountValue = 0m, AfterDiscount = x.Total, Location = "أماكن الأصناف", Note = ""
        }).ToList();
        UpdateTotal();
    }
    ...
    {
        var subtotal = _items.Where(x => x.ProductId > 0).Sum(x => x.Total);
        var total = Math.Max(0m, subtotal - _discount.Value);
        var cost = _items.Where(x => x.ProductId > 0).Sum(x => x.CostPrice * x.Quantity);
        var profit = total - cost;
        _totalLabel.Text = $"الإجمالي: {total:N2}"; _costLabel.Text = $"تكلفة الفاتورة: {cost:N2}"; _profitLabel.Text = $"قيمة ربح الفاتورة: {profit:N2}";
        _invoiceCountLabel.Text = _items.Count(x => x.ProductId > 0).ToString();
    }

    private void SaveOrder()
    {
        var validItems = _items.Where(x => x.ProductId > 0).ToList();
        if (validItems.Count == 0) { MessageBox.Show(this, "أضف صنفًا واحدًا على الأقل.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
        var customerId = Convert.ToInt64(_customer.SelectedValue ?? 0);
        if (_paymentStatus.Text == "آجل" && customerId == 0) { MessageBox.Show(this, "لا يمكن حفظ فاتورة آجلة بدون اختيار عميل.", "تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Warning); return; }
        try
        {
            var orderId = _service.SaveOrder(customerId == 0 ? null : customerId, _paymentStatus.Text, _discount.Value, _notes.Text, validItems);
            MessageBox.Show(this, $"تم حفظ الفاتورة بنجاح.\nرقم العملية: {orderId}", "تم الحفظ", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ClearCurrentInvoice(); _productSearch.Focus();
        }
        catch (Exception ex) { MessageBox.Show(this, $"تعذر حفظ الفاتورة:\n{ex.Message}", "خطأ", MessageBoxButtons.OK, MessageBoxIcon.Error); }
    }

    public void TriggerSave() => SaveOrder();
    public void FocusProductEntry() => _productSearch.Focus();
    public void AddLine() => AddNewInvoiceLine();
    ...
    public void DeleteSelectedLine()
    {
        ...
        _items.RemoveAt(_grid.CurrentRow.Index); RefreshGrid(); _productSearch.Focus();
    }

    public void ClearCurrentInvoice()
    {
        _items.Clear(); _discount.Value = 0M; _notes.Clear(); _productSearch.Clear(); _quantity.Value = 1M; RefreshGrid(); _productSearch.Focus();
    }
